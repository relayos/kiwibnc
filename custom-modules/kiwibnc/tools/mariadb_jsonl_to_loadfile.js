...
const DEFAULT_BUFFER_NORMALIZE = 'lower';
...
async function main() {
    ...
    if (args.header) {
        outputStream.write(loadDataSql(args.table || DEFAULT_TABLE) + '\n');
    }
    ...
    try {
        for await (const line of rl) {
            ...
            if (args.progress > 0 && total % args.progress === 0) {
...
function loadDataSql(table) {
    validateIdentifier(table);
    return `LOAD DATA LOCAL INFILE 'messages.tsv' INTO TABLE \`${table}\` ` +
...
function printUsage() {
    console.log('Convert message JSONL to a LOAD DATA file for bnc_messages.');
    console.log('Usage: node kiwibnc/tools/mariadb_jsonl_to_loadfile.js [--input messages.jsonl] [--output messages.tsv] [--buffer-normalize lower|alnum|none]');
    console.log('Options:');
    ...
    console.log('  -n, --buffer-normalize    Match message_store_mariadb.buffer_normalize (default: lower)');
    console.log('  -t, --table               Target table name (default: bnc_messages)');
    console.log('  -p, --progress            Log progress every N lines (default: 100000, 0 to disable)');
    console.log('      --header              Print a LOAD DATA statement before the rows');
